...
This is when we for some reason will deny the payment before completing the order for some reason. For example in Paypal; in order to streamline it nicely for the customer and show the proper amount directly, we need to validate that the country we got from Paypal is the same one as we have on the order.
We send:
